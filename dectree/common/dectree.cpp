...
void DecTree::depthFullPad() {
    // DecTree::Node* tmp1;
    // DecTree::Node* tmp2;
    for(uint32_t i = 0; i < this->node_vec.size(); ++i) {
        if (this->node_vec[i]->leaf) {
            if (this->node_vec[i]->level != this->depth) {
                DecTree::Node *tmp1 = new DecTree::Node();
                tmp1->parent = this->node_vec[i]->parent;
                tmp1->level = this->node_vec[i]->level;
                tmp1->leaf = this->node_vec[i]->leaf;
                tmp1->threshold = this->node_vec[i]->threshold;
                tmp1->attribute_index = this->node_vec[i]->attribute_index;
                tmp1->classification = this->node_vec[i]->classification;

                DecTree::Node *tmp2 = new DecTree::Node();
                tmp2->parent = this->node_vec[i]->parent;
                tmp2->level = this->node_vec[i]->level;
                tmp2->leaf = this->node_vec[i]->leaf;
                tmp2->threshold = this->node_vec[i]->threshold;
                tmp2->attribute_index = this->node_vec[i]->attribute_index;
                tmp2->classification = this->node_vec[i]->classification;

                this->node_vec[i] = new DecTree::Node();
                this->node_vec[i]->parent = tmp1->parent;
                this->node_vec[i]->level = tmp1->level; //leaf is false, threshold is 0, attribute_index is 0
                ...
                tmp1->level++;
                tmp1->parent = this->node_vec[i];
                tmp2->level++;
                tmp2->parent = this->node_vec[i];
                this->node_vec[i]->left = tmp1;
                this->node_vec[i]->right = tmp2;
                this->node_vec.push_back(tmp1);
                this->node_vec.push_back(tmp2);
            }
        }
    }
}
