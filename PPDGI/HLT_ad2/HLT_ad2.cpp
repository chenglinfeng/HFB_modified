...
vector<unsigned char> unsigned_char_arr_to_vec(unsigned char *arr, unsigned int &arr_len) {
...
void send_unsigned_char_vec(vector<unsigned char> &s, tcp::iostream &conn){
    int32_t sze = s.size();
    conn << sze << '\n';
...
void vec_to_unsigned_char_arr(unsigned char *arr, unsigned int &arr_len, vector<unsigned char> s){
    arr_len = s.size();
    //arr = new unsigned char[arr_len];
    for(int i=0;i<arr_len;i++){
        arr[i] = s[i];
    }
}

void receive_unsigned_char_vec(vector<unsigned char> s, tcp::iostream &conn){
    int32_t sze;
    conn >> sze;
    s.resize(sze);
    for (size_t i = 0; i < sze; i++)
...
//    }
//    cout << endl;
//    cout << "unsigned char* convert to string: " << endl;
//    vector<unsigned char> s = unsigned_char_arr_to_vec(arr,arr_len);
//    for(int i = 0; i < s.size(); i++){
//        cout<< hex << (unsigned int)s[i];
//    }
//    cout << endl;
//    send_unsigned_char_vec(s,conn);
//    cout << "all send" << endl;
//    cout << endl;
...
//    vector<unsigned char> s;
//    receive_unsigned_char_vec(s,conn);
//    for(int i = 0; i < s.size(); i++){
//        cout<< hex << (unsigned int)s[i];
//    }
//    cout << endl;
//    cout << "string convert to char* : " << endl;
//    vec_to_unsigned_char_arr(arr,arr_len, s);
//    for(int i = 0; i < arr_len; i++){
//        arr[i] = s[i];
//        cout<< hex << (unsigned int)arr[i];
//    }
...
SealTable gen_seal_table(DecTree tree, string dectree_filename){
...
//    t.load_packing_value_string(packing_value_en, packing_value_en_len);
    ...
    unsigned char *pack = new unsigned char[one_ot_size];
    unsigned int pakc_len = one_ot_size;
    memset(pack, 0, one_ot_size);
